...
const EventView = async ({ params }: { params: { eventId: string } }) => {
	...
	const formatOrders = (orders: any[]) => {
		...
		const ticketPrices = {} // Stores total price for each unique ticket name
		...
		orders.forEach((order) => {
			order.OrderItem.forEach((item: any) => {
				...
				// Update the total price for the ticket name (regardless of order)
				ticketPrices[ticketName] = (ticketPrices[ticketName] || 0) + price
			})
		})

		// Convert ticketPrices to formatted entries and add to formattedOrders
		formattedOrders.push(
			...Object.entries(ticketPrices).map(([name, value]) => ({ name, value })),
		)
